...
async function main() {
    ...
    if (dwhData.tables_dwh) {
      dwhData.tables_dwh.forEach((table: any) => {
        docs.push(new Document({
          ...
          metadata: { id: `table_${table.nom_table}`, type: 'metadata_table', domaine: 'dwh_structure' }
        }));
      });
    }
    if (dwhData.regles_metier_globales) {
      dwhData.regles_metier_globales.forEach((regle: any, index: number) => {
        docs.push(new Document({
          ...
          metadata: { id: `regle_metier_${index}`, type: 'regle_metier', domaine: 'business_rules' }
        }));
      });
    }
  }
  ...
  if (fs.existsSync(tripletsPath)) {
    ...
    if (tripletsData.triplets) {
      tripletsData.triplets.forEach((triplet: any) => {
        docs.push(new Document({
          pageContent: `Question: ${triplet.question}\nSQL: ${triplet.sql}\nVisualisation: ${triplet.visualisation}\nIntention: ${triplet.intention}`,
          metadata: { id: String(triplet.id), type: 'triplet_fewshot', domaine: String(triplet.domaine || 'general') }
        }));
      });
    }
  }

  console.log(`[LangChain] Nombre de documents prêts à l'indexation : ${docs.length}`);
  ...
  console.log(`[LanceDB] Génération des embeddings et création automatique de la table via LangChain...`);
